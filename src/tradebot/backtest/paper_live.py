...
class PaperLiveCryptoBot:
    ...
    def run_once(self) -> str:
        ...
        if not self.state.open_position:
            ...
            candidates.sort(
                key=lambda result: result.combined_opportunity_score
            ...
            if candidates:
                top = candidates[0]
                top_score = top.combined_opportunity_score if top.combined_opportunity_score is not None else top.opportunity_score
        ...
        return (
            ...
            f"top={top_candidate} action={action} reason={reason} warnings={'; '.join(self.state.warnings) or '-'}"
        )
    ...
    def _exit_decision(self, latest_time: datetime | None) -> tuple[float, str]:
        ...
        if self.state.loops_completed - int(position.get("entry_loop", 0)) >= self.max_holding_loops:
