...
def evaluate_window(histories: dict[str, list[Candle]], window: TimeWindow, cash: float, model: CryptoSignalModel | None = None) -> RobustnessWindowResult:
    sliced = slice_histories(histories, window)
    result = CryptoPortfolioPaperTrader(cash=cash, model=model).run(sliced)
    return _window_result(window, classify_market_regime(histories, window), result)


def evaluate_robustness(folder: str | Path, cash: float = 100000.0, model: CryptoSignalModel | None = None) -> RobustnessReport:
    histories = load_crypto_histories(folder)
    windows = split_time_windows(histories)
    results = [evaluate_window(histories, window, cash, model=model) for window in windows]
