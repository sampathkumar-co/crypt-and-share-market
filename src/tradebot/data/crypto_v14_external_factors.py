...
def _post_json(body: dict[str, Any], *, timeout: float = 45.0, retries: int = MAX_RETRIES) -> Any:
        ...
        try:
            with urlopen(request, timeout=timeout) as response:  # noqa: S310 - fixed public API URL
                if response.status != 200:
                    raise ExternalFactorDataError(f"Hyperliquid returned HTTP {response.status}")
                return json.loads(response.read().decode("utf-8"))
        except (HTTPError, URLError, TimeoutError, json.JSONDecodeError, ExternalFactorDataError) as exc:
            last_error = exc
            if attempt < retries:
...
def fetch_v14_funding(symbol: str, start: date, end: date, destination: Path) -> SourceFile:
    coin = V14_HYPERLIQUID_COINS.get(symbol)
    if coin is None:
    ...
    for _ in range(100):
        rows = _post_json({"type": "fundingHistory", "coin": coin, "startTime": cursor, "endTime": end_ms})
        if not isinstance(rows, list):
            raise ExternalFactorDataError(f"Unexpected Hyperliquid funding response for {symbol}")
        if not rows:
            break
        stamps: list[int] = []
        for row in rows:
            try:
                ...
                raise ExternalFactorDataError(f"Malformed funding row for {symbol}: {row}") from exc
            stamps.append(stamp)
        ...
        if next_cursor <= cursor:
            raise ExternalFactorDataError(f"Hyperliquid pagination stalled for {symbol}")
        cursor = next_cursor
        if len(rows) < 500:
            break
    if not records:
        raise ExternalFactorDataError(f"No Hyperliquid funding records for {symbol}")
    first_date = datetime.fromtimestamp(min(records) / 1000, tz=timezone.utc).date()
    last_date = datetime.fromtimestamp(max(records) / 1000, tz=timezone.utc).date()
    if first_date > start or last_date < end:
        raise ExternalFactorDataError(f"Incomplete Hyperliquid funding interval for {symbol}: {first_date} to {last_date}")
    rendered = [
        {
    ...
    count, missing, first, last = _write_csv(destination, ["timestamp", "funding_rate"], rendered)
    return SourceFile(
...
def fetch_v14_external_bundle(out_dir: str | Path, start: date, end: date) -> ExternalFactorManifest:
    root = Path(out_dir)
    ...
    for symbol in V14_HYPERLIQUID_COINS:
        files.append(fetch_v14_funding(symbol, start, end, root / "hyperliquid" / f"{symbol}.csv"))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Fetch frozen v1.4 external factors")
    ...
    manifest = fetch_v14_external_bundle(args.out, start, end)
